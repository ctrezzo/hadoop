...
public class NMCacheUploaderSCMProtocolPBServiceImpl implements
    NMCacheUploaderSCMProtocolPB {

  private NMCacheUploaderSCMProtocol real;

  public NMCacheUploaderSCMProtocolPBServiceImpl(NMCacheUploaderSCMProtocol impl) {
    this.real = impl;
  }

  @Override
  public NotifySCMResponseProto notify(RpcController controller,
      NotifySCMRequestProto proto) throws ServiceException {
    NotifySCMRequestPBImpl request = new NotifySCMRequestPBImpl(proto);
    try {
      NotifySCMResponse response = real.notify(request);
      return ((NotifySCMResponsePBImpl) response).getProto();
